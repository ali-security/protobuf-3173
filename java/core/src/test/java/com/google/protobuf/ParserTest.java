...
public class ParserTest {
  ...
  @Test
  public void testParsePartialFrom_noExceptionOnGetExtension() throws Exception {
    TestAllExtensions partialMessage =
        TestAllExtensions.newBuilder()
            .setExtension(TestRequired.single, TestRequired.newBuilder().setA(123).buildPartial())
            .buildPartial();
    ExtensionRegistry registry = ExtensionRegistry.newInstance();
    UnittestProto.registerAllExtensions(registry);
    TestAllExtensions testMessage =
        TestAllExtensions.newBuilder()
            .mergeFrom(partialMessage.toByteString(), registry)
            .buildPartial();

    assertThat(testMessage.getExtension(TestRequired.single).getA()).isEqualTo(123);
    assertThat(testMessage.getExtension(TestRequired.single).hasB()).isFalse();
  }
